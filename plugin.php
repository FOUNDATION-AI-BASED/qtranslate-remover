...
class QT_Importer {
	...
	public function add_hooks() {
		...
		add_action( 'wp_ajax_qt_clean_count_ajx', array( $this, 'clean_count_ajx' ) );
		add_action( 'wp_ajax_qt_detect_langs_ajx', array( $this, 'detect_langs_ajx' ) );
	...
	function menu() {
        ...
        <div class="wrap">
			...
			<style>
				.qti-grid{display:grid;grid-template-columns:1fr;gap:16px;max-width:960px}
				.qti-card{background:#fff;border:1px solid #ccd0d4;border-radius:4px;box-shadow:0 1px 1px rgba(0,0,0,.04)}
				.qti-card .qti-card-header{padding:16px;border-bottom:1px solid #ccd0d4;font-weight:600}
				.qti-card .qti-card-body{padding:16px}
				.qti-row{display:flex;gap:12px;align-items:center;flex-wrap:wrap}
				.qti-progress{height:10px;background:#f0f0f1;border-radius:8px;overflow:hidden;margin:8px 0}
				.qti-progress-bar{height:100%;width:0;background:#2271b1;transition:width .3s ease}
				.qti-log{max-height:360px;overflow:auto;font-size:12px;background:#f6f7f7;padding:8px;border:1px solid #ddd;margin-top:8px}
				@media(min-width:782px){.qti-grid{grid-template-columns:1fr 1fr}}
			</style>
			...
			if ( ! empty( $language_names  ) ) {
				...
				if ( ! get_option( '_qt_importer_clean_has_run' ) ) {
					?>
                <div class="notice notice-error"><p><?php esc_html_e( 'Please save the qTranslate settings to the database first. Go to Languages Tab of Qtranslate settings Edit the languages that are active & enabled and update their names to something, save and revert back.', 'qt-import' ) ?></p></div><?php
				}
			}
			...
			<?php if ( ! defined( 'ICL_SITEPRESS_VERSION' ) || version_compare( ICL_SITEPRESS_VERSION, '2.0.5', '<' ) ): ?>
				...
				<?php if ( get_option( '_qt_importer_clean_has_run' ) ): ?>
                    ...
                    <div class="notice notice-success"><p><?php _e( 'The clean up script has already run.', 'qt-import' ); ?></p></div>

				<?php else: ?>
                    <div class="notice notice-info"><p><?php _e( 'WPML is not enabled on this site, so only one language can remain. Please choose which language to keep.', 'qt-import' ) ?></p></div>

                    <form id="qt_clean_form" method="post" class="qti-grid">
						<?php wp_nonce_field( 'qt_clean', 'qt_do_clean' ); ?>
						<div class="qti-card">
							<div class="qti-card-header"><?php _e( 'Cleanup', 'qt-import' ); ?></div>
							<div class="qti-card-body">
								<div class="qti-row">
									<label><?php _e( 'Keep this language:', 'qt-import' ); ?></label>
									...
									<input type="text" name="language_keep_input" placeholder="en, pt-br" />
									<button type="button" id="qt_clean_detect_langs" class="button"><?php _e( 'Detect languages', 'qt-import' ); ?></button>
									<button type="button" id="qt_clean_detect" class="button"><?php _e( 'Detect leftovers', 'qt-import' ); ?></button>
									<span id="qt_clean_detect_result"></span>
								</div>
					...
					<div id="qt_clean_progress" class="qti-progress" style="display:none;">
						<div id="qt_clean_progress_bar" class="qti-progress-bar"></div>
					</div>
                    ...
                    <div id="qt_clean_status" class="qti-log" style="display:none;"></div>
	...
	function clean_ajx() {
		...
		if ( $posts ) {
			...
			$this->response['processed_batch'] = count( $posts );
			foreach ( $posts as $post_id ) {
			...
			// cleanup temporary data and qTranslate settings
	...
	function detect_langs_ajx() {
		...
		$rows = $wpdb->get_results( "
            SELECT post_title, post_content, post_excerpt FROM {$wpdb->posts} p 
            WHERE {$where} ( post_title LIKE '[:%' OR post_title LIKE '<!--:%' OR post_content LIKE '[:%' OR post_content LIKE '<!--:%' ) AND p.post_type NOT IN ('attachment', 'nav_menu_item', 'revision')
            LIMIT 1000
        " );
		$langs = [];
		foreach ( $rows as $row ) {
			foreach ( [ $row->post_title, $row->post_content, $row->post_excerpt ] as $frag ) {
				if ( ! $frag ) { continue; }
				$frag = preg_replace_callback( '#<!--:--><!--:([a-zA-Z0-9_\-]{2,10})-->#', function ( $found ) {
				...
				preg_match_all( '#\[:([a-zA-Z0-9_\-]{2,10})\]#i', $frag, $matches );
				if ( ! empty( $matches[1] ) ) {
		...
		echo json_encode( [ 'languages' => array_keys( $langs ) ] );
		exit;
	}

	function regex_fragment( $fragment ) {
		$fragment = preg_replace_callback( '#<!--:--><!--:([a-zA-Z0-9_\-]{2,10})-->#', function ( $found ) {
			return '[:' . strtolower( $found[1] ) . ']';
		}, $fragment );
		...
		if ( false !== strrpos( $fragment, '[:]' ) && 3 == strlen( $fragment ) - strrpos( $fragment, '[:]' ) ) {
			$fragment = substr( $fragment, 0, strlen( $fragment ) - 3 );
		}
		preg_match_all( '#\[:([a-zA-Z0-9_\-]{2,10})\]#i', $fragment, $language_codes );
		$language_codes = array_map( function ( $code ) {
			return str_replace( '_', '-', strtolower( $code ) );
		}, $language_codes[1] );
		$text_contents = preg_split( '#\[:[a-zA-Z0-9_\-]{2,10}\]#i', $fragment );
		array_shift( $text_contents );
	...
	function clean_post( $post_id, $language ) {
		...
		if ( $post ) {
			...
			$chosen = $language;
			if ( empty( $langs[ $chosen ] ) ) {
				if ( ! empty( $this->qt_default_language ) && ! empty( $langs[ $this->qt_default_language ] ) ) {
					$chosen = $this->qt_default_language;
				} else {
					$_keys = array_keys( $langs );
					if ( ! empty( $_keys ) ) {
						$chosen = $_keys[0];
					}
				}
			...
			if ( ! empty( $langs[ $chosen ] ) ) {
				$post['post_title']   = isset( $langs[ $chosen ]['post_title'] ) ? $langs[ $chosen ]['post_title'] : $post['post_title'];
				$post['post_content'] = isset( $langs[ $chosen ]['post_content'] ) ? $langs[ $chosen ]['post_content'] : '';
				if ( isset( $langs[ $chosen ]['post_excerpt'] ) ) {
					$post['post_excerpt'] = $langs[ $chosen ]['post_excerpt'];
				}
			}
			...
			if ( ! empty( $langs[ $chosen ]['custom_fields'] ) ) {
				foreach ( $langs[ $chosen ]['custom_fields'] as $k => $v ) {
